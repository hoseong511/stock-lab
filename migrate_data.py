...
def collect_stock_info():
    ...
    for code in target_code:
        print(code)
        time.sleep(1)
        result_price = ebest.get_stock_price_by_code(code, "100")
        ...
        result_short = ebest.get_short_trend_by_code(code, sdate="20200301", edate=today)
        if len(result_short) > 0:
            print(result_short)
            mongodb.insert_items(result_short, "stocklab", "short_info")

        result_agent = ebest.get_agent_trend_by_code(code, fromdt="20200301", todt=today)
